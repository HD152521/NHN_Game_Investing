...
function substep(
  ...
  if (baseHp <= 0) {
    ...
    /*
     * ★★ 적 본진 파괴는 **즉시 승리가 아니다** — 되돌린 결정이다 (2026-08-05) ★★
     *
     * 잠깐 "부수면 즉시 승리"였다. 대칭이 아름다웠지만 **실측이 설계를 무너뜨렸다**:
     *
     *   유닛만 · 통신원   8기 · 480 G · 첫 실점 없음 · 잔여 HP 100 · 클리어
     *   타워만 · 기본4+대공2 Lv2      1,800 G · 잔여 HP 80 · 클리어
     *
     * 정직하게 방어한 쪽이 **4배 비싸고 결과도 나빴다.** 예산의 21%만 쓴 유닛 러시가
     * 무피해로 이기면서 클리어율이 R1 0.316 → 0.632로 뛰었고, **13웨이브를 막는다는
     * 코어 루프가 통째로 우회됐다** — 웨이브가 시작되기도 전에 판이 끝나기 때문이다.
     *
     * `ENEMY_BASE_DPS`(45) 반격은 "소수의 상시 압박은 태우고 자금이 든 밀어붙이기만
     * 버틴다"로 설계됐는데, 통신원 8기(480 G)가 이미 그 문턱을 넘었다. 문턱이 의도보다
     * 훨씬 낮았다.
     *
     * ⚠️ `ENEMY_BASE_HP`를 올려 막지 마라 — 그 상수의 주석이 명시적으로 금지한다
     * (막아야 할 것은 총량이 아니라 지속성이고, 올리면 정상 플레이로도 못 뚫는 값이 된다).
     *
     * 그래서 **PRD FR-8.2 원안으로 되돌린다: 파괴는 승리가 아니라 정산 +1점 보너스다.**
     * 적 본진은 여전히 때릴 수 있고 부술 수 있으며(요새가 배경 그림이 아니다),
     * `settlement.ts`의 `enemyBaseDestroyed`가 그 성취를 점수로 받는다 — 줄곧 `false`라
     * 아무도 받을 수 없던 +1점이 이제 실제로 열린다(§13-1의 공백이 여기서 닫힌다).
     *
     * 이 분기는 `false`로 막아 두되 지우지 않는다: 다시 열려면 **왜 위 실측이 달라졌는지**를
     * 먼저 대답해야 한다.
     */
    phase = 'cleared';
  } else if (
