...
export const STAGES: Readonly<Record<StageId, StageConfig>> = {
  ...
  R2: {
    ...
    waveTable: {
       ...
       *
       * ★★ ×1.20 → **×1.15** (2026-08-05, 타워 Lv2 −15%와 한 묶음) ★★
       * 계수가 틀렸던 게 아니라 **분모가 바뀌었다** — 플레이어 산출을 깎았으면 지역 계수는
       * 다시 뽑아야 한다(§19-9: 비율이 아니라 실측 클리어율로 정한다). 타워 하향 직후 R2는
       * ×1.20에서 클리어율 **0.053**(19종 중 1종)까지 떨어져 R1(0.211)과 두 계단 벌어졌다.
       * 계수 스윕 실측(`npm run combat-sim` 예산 100%):
       * ```
       *   ×1.10 → 0.211 / 평균 잔여 HP 8.0   ← R1과 동률. 램프 정의 ②가 죽는다
       *   ×1.15 → 0.158 / 평균 잔여 HP 3.8   ★ 채택
       *   ×1.20 → 0.053 / 평균 잔여 HP 2.7   ← R3와 동률이 되어 아래 칸이 사라진다
       * ```
       * 총 적 HP 24,186 → **23,195**(웨이브 합 22,160 + 보스 1,035)로, R1(20,155)보다 크다는
       * 램프 정의 ①은 그대로다.
       */
      baseHp: scaleWaveHp(WAVE_BASE_HP_R1, 1.15),
      airWaves: AIR_WAVES_ALL,
    },
  ...
  R3: {
    ...
    waveTable: {
       ...
       *
       * ★★ ×1.38 → **×1.30** (2026-08-05, 타워 Lv2 −15%와 한 묶음) ★★
       * ⚠️ 위 문단이 적는 ×1.30 · 26,226이 다시 **현재 값**이 됐다 — 2026-08-04에 램프 역전을
       * 고치려고 ×1.38(27,815)로 올렸던 것을 되돌린 것이며, 그때의 목적(R3가 R2보다 어려울 것)은
       * 이번에도 그대로 성립한다. 되돌리는 근거는 하나뿐이다: **타워 산출을 15% 깎았으므로
       * 같은 계수는 더 이상 같은 난이도가 아니다.**
       *
       * ×1.38을 유지하면 R3는 **클리어율 0.000**이다(v1.4 ×2.68 사고와 같은 절벽). 계수 스윕:
       * ```
       *   ×1.15 → 0.158 / 평균 잔여 HP 3.8   ← R2와 동률
       *   ×1.20 → 0.053 / 평균 잔여 HP 2.7
       *   ×1.30 → 0.053 / 평균 잔여 HP 2.3   ★ 채택 — 여유가 R2보다 확실히 얇다
       *   ×1.38 → 0.000                      ← 아무도 못 깬다
       * ```
       * ★ ×1.20과 ×1.30은 클리어율이 같아 **평균 잔여 HP로 갈랐다**(2.7 vs 2.3). §16-4가
       * "클리어 수는 양자화돼 있으니 잔여 HP를 보조 지표로 함께 보라"고 남긴 교훈 그대로다.
       *
       * ★★ 그리고 이번 회차에서 R3의 유일한 클리어가 **`혼합 · 타워6 + 사환`으로 바뀌었다** ★★
       * 예산이 가장 큰 지역에서 타워 전용 조합이 전멸하고 유닛 혼합만 남는다 —
       * "타워 독점"이 실제로 깨진 지점이며, 그 대가로 유닛 비용을 내렸다
       * (`constants.ts` `UNIT_COST` 주석).
       */
