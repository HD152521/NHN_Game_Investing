 ...
 * 읽기 쉬운 환율을 만든다.
 *
 * 실효 비용을 골드로 환산하면 `S-01`과 같은 급이다 — 이것이 150을 고른 진짜 이유다:
 *   세션 총 투입 S = 11,900, 시작 AUM = 2,000 → 원금 1 AUM은 세션 동안 5.95회 투입된다.
 *   150 AUM을 태우면 투입액이 150 × 5.95 ≈ 893 줄고,
 *   R1 목표 수익률 ρ = 0.20에서 벌지 못하게 되는 골드는 893 × 0.20 ≈ **179 G**.
 *   → `S-01`의 200 G와 거의 같다. 재화만 다르고 무게는 같은 두 스킬이 된다.
 *
 * 쿨다운 60초(= 웨이브 2주기)는 남용 상한을 만든다: 13웨이브 390초 동안 최대 7회이므로
 * 실드가 덮는 시간은 7 × 8초 = 56초로 **스테이지의 14%**뿐이고, 최대 소모 AUM은
 * 1,050(시작 2,000의 52.5%)이다. 실드로 본진을 계속 막아 버리는 플레이가 성립하면 전투가
 * 사라지므로, 상한은 지속시간이 아니라 쿨다운으로 건다.
 */
