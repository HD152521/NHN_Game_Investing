 ...
 *
 * ⚠️ 2026-08-02 개정 — 여기 나오는 좌표는 **범위가 아니라 진원(epicenter)**이다.
 *    `S-01`·`S-02`는 실제로는 맵 전체에 걸리는 스킬이라, 렌더러(`draw-skill-fx.ts`)가
 *    이 점을 **출발점**으로 삼아 화면 폭 전체까지 연출을 펼친다. y와 `S-03`의 x는
 *    렌더러가 전장 좌표(지상 레인 / 사옥 앞)로 덮어쓴다 — 그쪽이 규칙이 정한 자리이기
 *    때문이다. 이 표에 남은 역할은 **가산 합성 포화를 피하는 x 분산**이다.
 *
 * - `S-01` 공시 폭탄: 광역 즉발이라 전장 한가운데(교전선)에서 시작해 좌우로 퍼진다.
 * - `S-02` 배당 살포: 아군이 나오는 좌측(사옥 쪽)에서 시작해 전선까지 번진다.
 * - `S-03` 서킷브레이커: "기지 앞 반원 돔"이므로 가장 왼쪽(시트 §08 명시).
 */
