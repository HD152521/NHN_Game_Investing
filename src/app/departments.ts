...
export const DEPARTMENTS: readonly DepartmentSpec[] = [
  ...
  {
    id: 'legal',
    // `effect`가 두 축을 말한다 — 청산선만 적어 두면 화면이 부서의 절반을 숨긴다(§16-6).
    // ⚠️ `display`에 두 숫자를 다 넣으면 `.dept__values`(flex·nowrap, 카드 최소 190px)를
    //    넘친다. 레벨별 수수료를 화면에 띄우려면 company.ts에 보조 줄이 필요하다.
    name: '법무팀',
    effect: '청산선 완화 · 거래 수수료',
    costs: [1500, 3500],
...
/**
 * 법무팀 — 강제 청산선 (FR-11.2 · O-8) **+ 거래 수수료 감면**.
 *
 * 기본값을 `DEFAULT_POSITION_PARAMS`에서 읽는다 — 표에 1.00이라 적혀 있어도 그 상수가
 * 바뀌면 Lv1이 따라가야 한다. 완화분만 여기서 더한다(합연산).
 *
 * ★★ 왜 효과가 둘이 됐는가 — 실측이 "실력이 늘면 이 부서가 죽는다"를 보여줬다 ★★
 * (2026-08-05 `bot-sim` R1 · 120판, GAME.md §16-6)
 *
 * | 청산선 | 손절 규율 **없음** ρ / 강제청산율 | 손절 규율 **있음** ρ / 강제청산율 |
 * |---|---|---|
 * | 1.00 (Lv1) | −2.1% / 28.4% | +2.2% / 0.1% |
 * | 1.30 (Lv3) | +0.8% / 17.5% | +2.2% / 0.0% |
 *
 * 손절을 하면 애초에 강제청산에 걸리지 않으므로 **청산선을 아무리 완화해도 바뀌는 것이
 * 하나도 없다**. 즉 청산선 완화는 "나쁜 결과가 났을 때만" 지불되는 **조건부 보험**이고,
 * 게임이 가르치려는 습관(손절)을 익힐수록 가치가 0으로 수렴한다. v1.4가 실력 축을
 * **리스크 관리**로 선언한 것과 정면으로 어긋난다.
 *
 * 게다가 나머지 네 부서(데스크·R&D·인사·IR)는 전부 **상시 가산**이라 결과와 무관하게
 * 항상 값을 한다. 법무팀만 조건부인 것은 밸런스 문제이기 이전에 **구조적 이질성**이다.
 *
 * → §16-6의 3안 중 **3안(청산선 완화 + 소액의 상시 이득)**을 골랐다. 청산선의 의미는
 *   그대로 두고(`src/position/`이 그 값을 쓰고 불변식 테스트가 지킨다), 부서가 **무엇을
 *   더 주는가**만 늘렸다. 1안(효과 교체)은 초보 보험을 통째로 없애 28.4% → 17.5%의 실측
 *   이득을 버리는 것이고, 2안(문구만)은 숙련자 가치 0을 그대로 남긴다.
 *
 * ★ 왜 하필 수수료인가 ★
 * 수수료는 이 게임에서 **유일하게 전략과 무관하게 확정적으로 나가는 비용**이다.
 * AUM은 일방통행 파이프(청산 대금은 골드로만 나간다)라 세션 총 투입 `Σstake ≤ S`가
 * 못 박혀 있고, 수수료는 `Σstake × feeRate`이므로 **손절을 하든 안 하든 총액이 같다.**
 * 조건부 보험의 정확한 반대편이라 §16-6이 요구한 "숙련자에게도 0이 아니다"를 만든다.
 *
 * 크기(정직하게): R1 기준 `S = 2,000 + 150×13 = 3,950`, 실측 소진율 93% →
 * `Σstake ≈ 3,674`. Lv3의 절감 0.8%p → 29 AUM → 골드 환산(×0.50) **약 +15 G/판**.
 * Lv2는 +7 G. 청산선 완화의 초보 이득(총골드 2,118 → 2,171 = +53 G)의 30% 수준인
 * **소액**이며, 그것이 §16-6이 요구한 크기다. ⚠️ 이 부서가 비용(1,500/3,500) 대비
 * 여전히 가장 약한 부서라는 사실은 바뀌지 않는다 — 가격 재조정은 PM 판단이다.
 *
 * ★ 왜 비율 감면인가 (절대 감면이 아니라) ★
 * 수수료가 0이 되면 `E[ρ] ≤ −feeRate`(§19-11, 마팅게일 경로에서 정리로 성립)가
 * `E[ρ] ≤ 0`으로 무너져 "매매는 기대적으로 손해"라는 전제가 사라진다. 비율로 두면
 * 기본 수수료가 양수인 한 **어떤 레벨에서도 수수료가 양수**임이 구조적으로 보장된다.
 * 레벨 간 감면 폭은 균등(0.4씩)이라 FR-11.3(곱연산 금지 — 실효 증가폭이 커지면 안 된다)도
 * 그대로 지킨다. 반환하는 것은 **완성된 요율**이지 호출부가 곱할 배수가 아니다.
 *
 * ★ 검토했다가 버린 후보들 (같은 것을 다시 시도하지 않도록) ★
 * - `minHoldMs` 단축: 봉이 1초 단위라 가격이 봉 안에서 안 변한다. 2,000ms→[1,000,2,000)은
 *   한 봉 이득이지만 그 아래는 전부 같고, 1,000ms 미만은 **진입가 그대로 즉시 왕복**이라
 *   세탁 통로가 된다. Lv2 = Lv3이 되거나 익스플로잇이 되거나 둘 중 하나다.
 * - `payoutBase`·`zMax` 상향: 크기가 z에 비례한다. 마팅게일에서는 z≈0이라 무의미하고
 *   실제 합성 차트(§19-11, surge/plunge가 ±31σ)에서는 폭발한다. 게다가 "크게 걸어
 *   크게 먹기"를 보상해 리스크 관리 축과 반대로 간다.
 * - `GOLD_CONVERSION` 상향: 세탁을 막는 장치가 이것 **하나뿐**이고(§8-2), 게다가
 *   `PositionParams`가 아니라 `trade.ts` 내부 상수라 시그니처를 바꿔야 한다.
 * - `requiredSpend` 감면: §19-8이 "난이도를 필요지출로 만들지 마라"라고 못박은 축이고,
 *   총골드가 게이트에 아슬아슬한 현 상태에서는 "소액"이 아니라 클리어 여부를 뒤집는다.
 *
 * ⚠️ 두 값 모두 진입 시점에 포지션 개체로 **복사**된다(`OpenPosition.liqLine` / `.fee`).
 * 그래서 "진행 중 업그레이드가 기존 포지션에 소급되지 않는다"가 자동으로 성립한다 —
 * 타워·유닛 스탯과 같은 규율이다(§17-2 "스탯은 개체에 실린다").
 */
...
/** 레벨별 수수료 **감면율**. 기본 요율에 `1 − 이 값`을 곱해 최종 요율을 만든다. */
const LEGAL_FEE_DISCOUNT: readonly [number, number, number] = [0, 0.4, 0.8];

/**
 * 법무팀 — 진입 수수료 요율(최종값). 기본 1.00% → Lv2 0.60% → Lv3 0.20%.
 *
 * `liquidationLineFor`와 같은 이유로 기본값을 상수에서 읽는다. 밸런스 상수 자체는
 * `src/position/constants.ts`가 소유하며 이 파일은 **읽기만 한다.**
 */
export function tradeFeeRateFor(levels: DepartmentLevels): number {
  return DEFAULT_POSITION_PARAMS.feeRate * (1 - LEGAL_FEE_DISCOUNT[levels.legal - 1]!);
}
