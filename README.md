...
PLAN STEP 기준. 이미지 생성이 필요한 STEP 1~7·12 는 아직 미착수입니다.

| STEP | 내용 | 상태 |
|---|---|---|
| 0 | 스캐폴딩 · 스타일 앵커 고정 | ✅ |
| 1~7 | 에셋 생성 (GATE-A/B/C 포함) | ⬜ 수작업 단계 |
| 8 | 에셋 후처리 파이프라인 | ✅ `tools/asset-pipeline/` |
| 9 | 색 시스템 · 색약 모드 | ✅ `src/design/`, `src/styles/` |
| 10 | 파츠 분리 애니메이션 | ✅ `src/anim/`, `src/render/` |
| 11 | QA — 자동 검사분 | ✅ 육안 판정 항목은 에셋 대기 |
| 12 | P1 잔여 에셋 | ⬜ |

테스트 326개 통과 / 파이프라인 커버리지 95.8% stmts.
...
- **UP(`#FF4D5A`) vs DOWN(`#2E86FF`) 명도 대비 1.08:1** — 색약 모드에서도 2.83:1 로 WCAG 3:1 미달입니다.
  `▲▼` 기호와 캔들 채움/빈칸은 보조가 아니라 **유일하게 신뢰 가능한 구분 신호**입니다.
- 색약 모드의 음영 토큰(`UP-DEEP`/`ENEMY-DEEP`)은 아트가이드에 미지정이라 채널 비율로 파생했습니다 (`#996A00`/`#00446B`). **아트 담당 확인 필요.**
- 스켈레톤의 팔다리 길이는 **임의값**입니다. 실제 스프라이트 비율로 재조정하지 않으면 팔다리가 분리돼 보입니다.
- E-02 파트 6개를 리그 슬롯 10개에 재사용합니다. **좌우 비대칭 장식(견장 등)이 있으면 반대쪽이 어색해집니다.**
- 애니메이션 벤치마크는 **CPU 포즈 계산만** 측정한 값입니다(0.31% 예산). 실제 60FPS 리스크는 draw call 수(프레임당 680~1,360)이며 브라우저에서만 측정됩니다.
